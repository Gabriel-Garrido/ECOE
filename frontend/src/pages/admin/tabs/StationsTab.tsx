...
import { getStations, createStation, updateStation, toggleStation } from '../../../api/stations'
...
export default function StationsTab({ exam }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: (d: { name: string; educator_name?: string; weight_percent: string; order: number }) =>
    ...
    onError: (e: unknown) =>
      alert(
        (e as { response?: { data?: { name?: string[]; detail?: string } } })?.response?.data
          ?.name?.[0] || 'Error al crear estación.'
      ),
  })
    ...
    <div>
      ...
      {stations.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
          <p className="text-gray-500">No hay estaciones. Agrega la primera.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50 text-left">
                ...
                <th className="px-4 py-3 font-medium text-gray-600 hidden md:table-cell">Educador</th>
                <th className="px-4 py-3 font-medium text-gray-600">Ponderación</th>
                <th className="px-4 py-3 font-medium text-gray-600">Estado</th>
                <th className="px-4 py-3 font-medium text-gray-600 hidden md:table-cell">Ítems</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody>
              {stations.map((station) => (
                <tr key={station.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <Link to={`/admin/exams/${exam.id}/stations/${station.id}`}>
                        <Button variant="secondary" size="sm">Pauta</Button>
                      </Link>
                      {!isClosed && (
                        <Button
                          variant="ghost"
...
function StationForm({
  ...
  onSubmit: (d: { name: string; educator_name?: string; weight_percent: string }) => void
  loading: boolean
  onCancel: () => void
  defaultValues?: Partial<{ name: string; educator_name: string; weight_percent: string }>
}) {
  ...
  return (
    <form
      ...
      className="space-y-4"
    >
      <Input label="Nombre de la estación" {...register('name', { required: true })} />
      <Input label="Nombre del educador (opcional)" {...register('educator_name')} />
      <Input
        label="Ponderación (%)"
        type="number"
        min="0"
