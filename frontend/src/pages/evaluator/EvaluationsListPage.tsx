...
import { getStationEvaluations, getOrCreateDraftEvaluation } from '../../api/evaluations'
...
export default function EvaluationsListPage() {
  ...
  const startEvalMutation = useMutation({
    mutationFn: (studentId: number) => getOrCreateDraftEvaluation(stationIdNum, studentId),
    onSuccess: (evaluation) => {
      navigate(`/evaluador/evaluaciones/${evaluation.id}`)
    },
    onError: () => alert('Error al iniciar la evaluación.'),
  })
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          {
            label: 'Sin iniciar',
            count: examStudents.filter((es) => !evalByStudent[es.student.id]).length,
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-left">
              <th className="px-4 py-3 font-medium text-gray-600">Estudiante</th>
              <th className="px-4 py-3 font-medium text-gray-600 hidden sm:table-cell">RUT</th>
              <th className="px-4 py-3 font-medium text-gray-600">Estado</th>
              <th className="px-4 py-3 font-medium text-gray-600 hidden md:table-cell">Nota</th>
          ...
          <tbody>
            {examStudents.map((es) => {
              ...
              return (
                <tr key={es.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium">{es.student.full_name}</td>
