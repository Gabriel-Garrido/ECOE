...
import type { EvaluationItemScore } from '../../types'
...
export default function EvaluateStudentPage() {
  ...
  const finalizeMutation = useMutation({
    ...
    onError: (e: unknown) => {
      const errs =
        (e as { response?: { data?: { incomplete_items?: { description: string }[] } } })
          ?.response?.data?.incomplete_items
      alert(
        errs
          ? `Ítems incompletos:\n${errs.map((i) => `• ${i.description}`).join('\n')}`
          : 'Error al finalizar.'
      )
    },
  })
  ...
  const triggerSave = useCallback(
    (scores: Record<number, { points: string; comment: string }>, comment: string) => {
      ...
      debounceRef.current = setTimeout(() => {
        ...
        updateMutation.mutate({ item_scores: itemScores, general_comment: comment })
      }, DEBOUNCE_MS)
    },
    [evaluation]
  )

  const handleScoreChange = (scoreId: number, field: 'points' | 'comment', value: string) => {
  ...
  if (!evaluation) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/evaluador/mis-estaciones" className="text-primary-600 hover:underline mt-2 block">
          Volver
        </Link>
      </div>
    )
  ...
  const sortedScores = [...evaluation.item_scores].sort(
    (a, b) => a.rubric_item_order - b.rubric_item_order || a.rubric_item - b.rubric_item
  )
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link to="/evaluador/mis-estaciones" className="hover:text-primary-600">
          Mis Estaciones
        </Link>
        <span>/</span>
        <span>Evaluar</span>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">
              ...
              {evaluation.items_completed}/{evaluation.items_total} ítems completados
            </p>
          </div>
          {isFinal && (
            <div className="text-right">
              ...
              <div className="text-sm text-gray-500">{evaluation.total_points_display} pts</div>
              <ApprovedBadge
                approved={
      ...
      <div className="space-y-3 mb-4">
        {sortedScores.map((score, idx) => {
          const local = localScores[score.id] ?? { points: score.points ?? '', comment: score.comment }
          const maxPts = parseFloat(score.rubric_item_max_points)
          const pts = parseFloat(local.points)
          const isValid = local.points === '' || (!isNaN(pts) && pts >= 0 && pts <= maxPts)
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-900">{score.rubric_item_description}</p>
                  <div className="flex items-center gap-3 mt-3">
                    <div>
                      ...
                      <input
                        ...
                        value={local.points}
                        disabled={!canEdit}
                        onChange={(e) => handleScoreChange(score.id, 'points', e.target.value)}
                        placeholder="0.00"
                      />
                  ...
                  <div className="mt-2">
                    ...
                    <textarea
                      ...
                      value={local.comment}
                      disabled={!canEdit}
                      onChange={(e) => handleScoreChange(score.id, 'comment', e.target.value)}
                      rows={2}
                    />
                  </div>
                </div>
      ...
      <div className="flex flex-wrap gap-3 justify-end">
        {isFinal && (
          <Button variant="secondary" loading={pdfLoading} onClick={handlePdfDownload}>
            Descargar PDF
          </Button>
        )}
        {!isFinal && (
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-700">
            Al finalizar, no podrás editar esta evaluación. ¿Confirmas la finalización?
          </p>
